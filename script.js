...
const PROJECTS = [
  {
    ...
    tags: ["ai", "backend", "ba"],
    badge: "Freelance", badgeType: "badge-green",
    date: "Mar – Apr 2026",
    live: "lunfa.net",
    stack: ["React 19", "Express 5", "MongoDB", "Vertex AI · Gemini 2.5", "JWT"],
    links: [
      { label: "Live site →", href: "https://lunfa.net" },
      { label: "Code", href: "https://github.com/giabaocode", ghost: true },
    ],
    flow: [
      { label: "USER INPUT", desc: "Learner types a sentence in Chinese (HSK 1–4 level)." },
      {
        label: "AI ANALYZE", desc: "Gemini 2.5 Flash evaluates the sentence against a grammar rulebook.",
        challenge: "The AI sometimes returns broken JSON — control characters inside string values crash the parser.",
      ...
      {
        label: "HIGHLIGHT", desc: "Wrong words are painted red inside the original sentence.",
        challenge: "Highlighting the exact wrong word without matching a shorter substring elsewhere.",
      ...
      {
        label: "SELF-CORRECT", desc: "The learner gets reason + correct structure + a prompt, and rewrites it themselves.",
        challenge: "Vertex AI requires the chat history to start with a 'user' role, and long history blows the token limit.",
      ...
      { label: "MASTERED", desc: "Correct sentence accepted. Conversation auto-titled by Gemini and saved to MongoDB." },
    ],
  },
  {
    id: "unihub",
    ...
    tags: ["backend", "ba"],
    badge: "Course", badgeType: "badge",
    date: "Apr – May 2026",
    note: "Load-tested with K6 at 12,000 virtual users · p95 < 2s · error < 1%.",
    stack: ["Java 21", "Spring Boot 3", "PostgreSQL", "Redis", "Resilience4j", "PWA"],
    links: [{ label: "Code", href: "https://github.com/giabaocode" }],
    flow: [
      {
        label: "REGISTER", desc: "Student claims a seat for a workshop.",
      ...
      {
        label: "QR PAYMENT", desc: "Pays via the SePay QR gateway; a webhook confirms asynchronously.",
        challenge: "If the payment gateway hangs, it could exhaust the thread pool and take the whole system down.",
        fix: "A Resilience4j circuit breaker trips OPEN after repeated timeouts → fail-fast instead of piling up blocked threads.",
      },
      { label: "EMAIL TICKET", desc: "A QR-coded ticket is emailed asynchronously via Spring events + @Async, so the API responds instantly." },
      {
        label: "OFFLINE CHECK-IN", desc: "Staff scan tickets at the venue through a PWA.",
      ...
      { label: "ATTENDED", desc: "Check-in recorded. SeatReleaseJob frees expired PENDING tickets back to the pool." },
    ],
  },
  {
    id: "auction",
    ...
    tags: ["backend", "ba"],
    badge: "Final project", badgeType: "badge",
    ...
    links: [{ label: "Code", href: "https://github.com/giabaocode" }],
    flow: [
      { label: "LISTED", desc: "Seller posts a product with a WYSIWYG editor and multi-image upload to Cloudinary." },
      {
        label: "BIDDING", desc: "Bidders place manual bids or set an auto-bid ceiling.",
        challenge: "Many people bid the same product at the same millisecond → price race condition.",
      ...
      {
        label: "ANTI-SNIPE", desc: "Guards against last-second sniping.",
        challenge: "Snipers bid in the final seconds to deny others a counter-bid.",
        fix: "If time-remaining < 5 minutes when a bid lands, end_at is pushed +10 minutes — the snipe strategy stops working.",
      },
      { label: "WON", desc: "Auction closes. A node-cron job runs every minute to detect ended auctions and fire 6 kinds of notification email." },
      {
        label: "PAID → SHIPPED", desc: "Buyer uploads payment proof, seller confirms shipping, buyer confirms receipt.",
      ...
      { label: "RATED", desc: "Both sides leave +/- reputation. Denormalized current_price & bid_count keep listing queries fast." },
    ],
  },
  {
    id: "coffee",
    ...
    tags: ["ba", "backend"],
    badge: "Running in a real shop", badgeType: "badge-amber",
    date: "Jun – Aug 2025",
    live: "in production",
    stack: ["React 19", "Express 5", "PostgreSQL", "Docker"],
    links: [{ label: "Code", href: "https://github.com/tdthien106/Coffee_shop" }],
    flow: [
      { label: "BROWSE", desc: "Staff browse drinks by category — the POS module I built end-to-end." },
      { label: "CART", desc: "Real-time cart with per-item notes (preset tags + free text)." },
      {
        label: "CHECKOUT", desc: "A 3-step checkout: Summary → Transfer → Success.",
        challenge: "Circular foreign keys: orders.payment_id → payment and payment.order_id → orders.",
      ...
      { label: "SUCCESS", desc: "Order completed and persisted; receipt issued." },
      {
        label: "DASHBOARD", desc: "Managers see revenue-by-hour, peak hours and period-over-period comparisons.",
        challenge: "Heavy analytics queries on the manager dashboard, plus keeping staff out of it.",
  ...
  },
  {
    ...
    tags: ["backend", "ba"],
    badge: "Team of 4", badgeType: "badge",
    date: "Mar – Apr 2026",
    stack: ["Next.js 16", "Prisma 7", "PostgreSQL", "TypeScript", "Zod"],
    links: [{ label: "Code", href: "https://github.com/giabaocode" }],
    flow: [
      {
        label: "DRAFT → CONSULT", desc: "The registration ticket I owned: a 6-state machine (DRAFT → CONSULTING → WAITING_VIEW → WAITLIST → COMPLETED / CANCELLED).",
      ...
      { label: "VIEWING", desc: "A viewing appointment is scheduled off the registration ticket." },
      {
        label: "DEPOSIT → CONTRACT", desc: "Deposit taken, then a contract is signed per bed.",
        challenge: "If admin changes a bed's price later, old contracts must not change.",
      ...
      {
        label: "RETURN", desc: "Move-out and reconciliation.",
  ...
  },
  {
    ...
    tags: ["backend", "ba"],
    badge: "Sole implementer", badgeType: "badge",
    date: "Nov 2025 – Jan 2026",
    ...
    flow: [
      {
        label: "BOOK", desc: "Customer books an exam / vaccination / spa slot.",
      ...
      { label: "EXAM", desc: "Doctor examines the pet and records a diagnosis." },
      {
        label: "PRESCRIBE → INVOICE", desc: "Prescription written, invoice generated automatically.",
        challenge: "Finishing an exam must atomically update status, write the prescription and create a correct invoice.",
      ...
      { label: "PAID", desc: "Payment recorded → loyalty points added, active service packages decremented per use." },
    ],
  },
  {
    id: "melodix",
    ...
    badge: "Mobile", badgeType: "badge-blue",
    date: "Mar – Apr 2026",
    stack: ["Java", "Android", "MVVM", "Supabase", "Room", "ExoPlayer"],
    links: [{ label: "Code", href: "https://github.com/giabaocode" }],
    flow: [
      {
        label: "PLAY", desc: "User taps a song; playback starts through ExoPlayer.",
      ...
      {
        label: "BACKGROUND", desc: "Music keeps playing when the app is backgrounded, with a media notification.",
      ...
      {
        label: "DOWNLOAD", desc: "Songs are downloaded for offline listening via WorkManager.",
        challenge: "Scoped Storage changed how files are written on Android 10+.",
      ...
      { label: "OFFLINE", desc: "Tracks play from the local Room library with no network." },
    ],
  },
];
...
function projectMarkup(p) {
  const nodes = p.flow
    .map((n, i) => nodeMarkup(n, i, p.id) + (i < p.flow.length - 1 ? '<span class="wire" aria-hidden="true"></span>' : ""))
    .join("");

  const first = detailMarkup(p.flow[0]);
  const stack = p.stack.map((s) => `<span>${s}</span>`).join("");
  const links = p.links
    .map((l) => `<a href="${l.href}" target="_blank" rel="noopener"${l.ghost ? ' class="ghost"' : ""}>${l.label}</a>`)
    .join("");
  const livePill = p.live
    ? `<span class="live-pill"><span class="sys-dot"></span>${p.live === "in production" ? "IN PRODUCTION" : "LIVE · " + p.live}</span>`
    : "";
  const noteRow = p.note ? `<p class="project-tagline" style="margin-top:10px;color:var(--text-3);font-family:var(--mono);font-size:12.5px">${p.note}</p>` : "";
